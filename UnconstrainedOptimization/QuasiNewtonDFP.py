...
def generate_points(x_start, f, grad, epsilon=1e-10, steps=100000):
    """
    ...
    for i in range(1, steps):
        ...
        H_new = H_old - (H_old * y * y.T * H_old) / (y.T * H_old * y) + (s * s.T) / (y.T * s)
        X = np.concatenate((X, x_new), axis=1)
        z_new = f(np.array(x_new))
        Z = np.concatenate((Z, z_new))
        H_old = H_new
...
if __name__ == "__main__":
    ...
    plot_2D_figure(X, Y, Z, x, y, './figures/DFPconvex2D.png')
    plot_3D_figure(X, Y, Z, x, y, z, './figures/DFPconvex3D.png')
    ...
    # plot_2D_figure(X, Y, Z, x, y, './DFProsenbrock2D.png')
    # plot_3D_figure(X, Y, Z, x, y, z, './DFProsenbrock3D.png')